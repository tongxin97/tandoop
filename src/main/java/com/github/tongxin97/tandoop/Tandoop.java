...
//    System.out.println("types: " + types);
  ...
  private void calculateMethodWeights(Map<String, Long> methodSelections, Map<String, Long> methodInvocations
    ...
    for (int i = 0; i < sizeM; i++) {
      ...
      double newWeight =  weightAlpha * uncovRatios.get(methodName) + (1-weightAlpha) * (1- succ/maxSucc);
      if (k > 0) {
...
//         System.out.printf("Instance val is null: %s.%s\n", method.ClassName, method.Name);
      ...
      if (numIterations == updateMethodWeightsInterval) {
        Map<String, Double> uncovRatios = new HashMap<>(); // placeholder
        calculateMethodWeights(methodSelections, methodInvocations, uncovRatios);
